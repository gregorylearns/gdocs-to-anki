...
def parse_md(unparsed_md):
    ...
    for line in unparsed_md.split("\n"):
        ...
        num_of_fields = len(formatted_fields)

        if num_of_fields > 1:
            parsed_product += f"{formatted_fields[-2]}|{formatted_fields[-1]}\n" # new ni
        elif num_of_fields == 1:
            print(f"-----> {type(formatted_fields)}")
            print(f"{formatted_fields[0]}")
            parsed_product += f"{formatted_fields[0]}|(merged cell)\n" # new ni
        elif num_of_fields == 0:
            continue #kapoya ani na bug HAHAH
